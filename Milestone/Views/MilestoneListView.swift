...
struct MilestoneListView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(item: $selectedMilestone, onDismiss: { self.selectedMilestone = nil }) { milestone in
    ...
    private var list: some View {
        ...
        List {
            ForEach(self.pinnedItem()) { pinnedItem in
                MilestoneView(folder: folder, milestone: pinnedItem)
                    .padding(.all, 4)
                    .onTapGesture {
                        self.selectedMilestone = pinnedItem
                    }
            }
    ...
    /**
     过滤置顶记录
     */
    private func pinnedItem() -> [Milestone] {
        return milestones.filter { $0.isPinned }
    }
    //
    //    /**
    //     展示新增里程碑弹窗
